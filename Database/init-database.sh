...
# Create the THINKON_ERP user and schema
echo "Creating THINKON_ERP user and schema..."
sqlplus -s sys/${ORACLE_PWD}@//localhost:1521/XE as sysdba <<EOF
-- Create user if not exists
DECLARE
  user_count NUMBER;
BEGIN
  SELECT COUNT(*) INTO user_count FROM dba_users WHERE username = 'THINKON_ERP';
  IF user_count = 0 THEN
    EXECUTE IMMEDIATE 'CREATE USER THINKON_ERP IDENTIFIED BY THINKON_ERP';
    EXECUTE IMMEDIATE 'GRANT CONNECT, RESOURCE, DBA TO THINKON_ERP';
    EXECUTE IMMEDIATE 'GRANT UNLIMITED TABLESPACE TO THINKON_ERP';
    DBMS_OUTPUT.PUT_LINE('User THINKON_ERP created successfully');
  ELSE
    DBMS_OUTPUT.PUT_LINE('User THINKON_ERP already exists');
  END IF;
END;
/
EXIT;
...
echo "=========================================="
echo "User: THINKON_ERP"
echo "Password: THINKON_ERP"
