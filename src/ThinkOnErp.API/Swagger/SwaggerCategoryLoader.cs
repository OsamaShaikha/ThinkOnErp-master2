...
public class SwaggerCategoryLoader
{
    ...
    private void LoadDefaultFallbackCategories()
    {
        ...
        var defaults = new List<(string Code, string Title, string Desc, int Order, string Controllers)>
        {
            ...
            (ApiCategories.Inventory, "8. Inventory & Trade Documents API", "Inventory management, items master, item categories (main/sub tree), BOM kits & assemblies, price lists, warehouses, stock movements, FIFO costing, and universal trade documents.", 8, "TrxDocumentsController,InvItemsController,InvCategoriesController,InvBomController,InvWarehousesController,InvStockController,InvOpeningBalancesController,InvModifiersController,InvPriceListsController"),
            (ApiCategories.Pos, "9. Point of Sale (POS) API", "Point of Sale operations: items catalog, till sessions, shifts, orders, split payments, offline sync, dining tables, reservations, KDS kitchen display, Z-reports, batch preps, gift cards, and analytics.", 9, "PosItemsController,PosShiftsController,PosOrdersController,PosSyncController,PosTablesController,PosReservationsController,PosKdsController,PosZReportController,PosBatchPrepController,PosGiftCardsController,PosAnalyticsController"),
            (ApiCategories.Hr, "10. HR & Dynamic Payroll Engine API", "Human Resources & Payroll operations: employee management, salary structures, leaves & absence tracking, work calendars, dynamic proration, overtime, social security contributions, progressive tax calculation, loan auto-capping, attendance corrections, variable adjustments, and balanced GL posting.", 10, "EmployeesController,SalaryComponentsController,LeavesController,WorkCalendarsController,AttendanceCorrectionsController,PayrollPoliciesController,LoansController,PayrollAdjustmentsController,PayrollController")
        };
