...
public interface IInvItemService
{
    ...
    Task<ApiResponse<List<InvItemListDto>>> GetAllAsync(string? search, long? categoryId, int pageNumber, int pageSize, CancellationToken cancellationToken = default);
    ...
    Task<ApiResponse<InvItemDto>> AddOpeningBalancesAsync(long itemId, List<ItemWarehouseOpeningBalanceDto> openingBalances, CancellationToken cancellationToken = default);
    Task<ApiResponse<List<ThinkOnErp.Application.DTOs.Pos.PosItemDto>>> GetPosItemsAsync(
        string? search,
        long? categoryId,
        bool onlyPosVisible,
        int pageNumber,
