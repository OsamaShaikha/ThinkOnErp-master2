...
public class GetUsersByBranchIdQueryHandler : IRequestHandler<GetUsersByBranchIdQuery, List<UserDto>>
{
    private readonly IUserRepository _userRepository;

    public GetUsersByBranchIdQueryHandler(IUserRepository userRepository)
    {
        _userRepository = userRepository ?? throw new ArgumentNullException(nameof(userRepository));
    }

    public async Task<List<UserDto>> Handle(GetUsersByBranchIdQuery request, CancellationToken cancellationToken)
    {
        var users = await _userRepository.GetByBranchIdAsync(request.BranchId);
